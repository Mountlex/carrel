...
actor ConvexClient {
    ...
    func signInWithEmail(email: String, password: String) async throws -> AuthTokens {
        try await requestWithoutAuth(
            .post,
    ...
    func signUpWithEmail(email: String, password: String) async throws -> AuthTokens {
        try await requestWithoutAuth(
            .post,
            "/api/mobile/auth/email/signup",
            body: ["email": email, "password": password]
        )
    }
